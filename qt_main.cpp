...
public:
    HotelMainWindow()
        : currentGuestIndex(-1)
    {
        setWindowTitle("Hotel Reservation System - Qt");
        resize(1400, 900);
...
private:
    ...
    void buildHomePage()
    {
        ...
        QLabel* intro = new QLabel(
            "This Qt interface is designed as a proper hotel operations dashboard.\n\n"
            "Guest Register creates a guest account.\n"
            "Guest Login opens the guest workspace for bookings, bills, and profile updates.\n"
            "Admin Login opens the operational workspace for rooms, bookings, billing, rules, and records."
        );
        ...
        homePage = buildPageShell(
            ...
            "Professional Qt workspace for guest services and hotel administration.",
            &guestLoginStatus,
            body
        );
        ...
        connect(btnGuestRegister, &QPushButton::clicked, this, [this]() {
            ...
            setWorkspace(guestRegisterWorkspace, "Create a guest account with complete profile details.");
        ...
        connect(btnGuestLogin, &QPushButton::clicked, this, [this]() {
            ...
            setStatusLabel(guestLoginStatus, "Ready for guest login.");
        ...
        connect(btnAdminLogin, &QPushButton::clicked, this, [this]() {
            ...
            setStatusLabel(adminLoginStatus, "Ready for admin login.");
    ...
    void buildGuestRegisterPage()
    {
        ...
        QGroupBox* formBox = createFormBox(
            "Guest Registration",
            QStringList() << "Username" << "Password" << "Guest ID" << "Full Name" << "Phone" << "CNIC" << "Address",
            guestRegisterFields,
            &formWrapper
        );
        ...
        guestRegisterPage = buildPageShell(
            "Guest Registration",
            "Create a guest account with profile details and secure login credentials.",
            &guestRegisterStatus,
            body
        );
    ...
    void configureGuestForm()
    {
        ...
        case 2:
            ...
            showGuestField(0, "Room Type");
            showGuestField(1, "Room Number");
            showGuestField(2, "Check-In Date");
            showGuestField(3, "Check-Out Date");
            showGuestField(4, "Total Days");
            setWorkspace(guestWorkspace, QString::fromStdString(hotel->getAvailableRoomsText()));
            setStatusLabel(guestDashboardStatus, "Enter booking details, then book the room.");
            break;
        case 3:
        ...
        case 5:
            ...
            showGuestField(0, "Bill ID");
            showGuestField(1, "Payment Method (1/2/3)");
            setWorkspace(guestWorkspace, QString::fromStdString(hotel->getGuestBillsText(currentGuestIndex)));
            setStatusLabel(guestDashboardStatus, "Use 1 for Cash, 2 for Card, 3 for Online.");
            break;
        case 6:
            ...
            setWorkspace(guestWorkspace, QString::fromStdString(hotel->getGuestInfoText(currentGuestIndex)));
            setStatusLabel(guestDashboardStatus, "Update your guest information.");
            break;
        case 7:
    ...
    void runGuestAction()
    {
        ...
        case 2:
            ok = hotel->bookRoomGui(
                ...
                guestFormFields[1]->text().toInt(),
                guestFormFields[2]->text().toStdString(),
                guestFormFields[3]->text().toStdString(),
                guestFormFields[4]->text().toInt(),
                message
            );
    ...
    void runAdminAction()
    {
        ...
        case 0:
            ...
            break;
        case 1:
            setWorkspace(adminWorkspace, QString::fromStdString(hotel->getGuestByIdText(adminFormFields[0]->text().toInt())));
            setStatusLabel(adminDashboardStatus, "Guest search completed.");
            break;
        case 2:
            ok = hotel->deleteGuestGui(adminFormFields[0]->text().toInt(), message);
        ...
        case 7:
            ...
            break;
        case 8:
            ok = hotel->checkInBookingGui(adminFormFields[0]->text().toInt(), message);
            setStatusLabel(adminDashboardStatus, QString::fromStdString(message));
            setWorkspace(adminWorkspace, QString::fromStdString(hotel->getAllBookingsText()));
            if (ok) clearEdits(adminFormFields);
            break;
        case 9:
            ok = hotel->checkOutBookingGui(adminFormFields[0]->text().toInt(), message);
            setStatusLabel(adminDashboardStatus, QString::fromStdString(message));
            setWorkspace(adminWorkspace, QString::fromStdString(hotel->getAllBookingsText()));
            if (ok) clearEdits(adminFormFields);
            break;
        case 10:
            ok = hotel->cancelBookingGui(adminFormFields[0]->text().toInt(), message);
            setStatusLabel(adminDashboardStatus, QString::fromStdString(message));
            setWorkspace(adminWorkspace, QString::fromStdString(hotel->getAllBookingsText()));
            if (ok) clearEdits(adminFormFields);
            break;
        case 11:
            ...
            break;
        case 12:
            ok = hotel->applyDiscountGui(adminFormFields[0]->text().toInt(), adminFormFields[1]->text().toDouble(), message);
            setStatusLabel(adminDashboardStatus, QString::fromStdString(message));
            setWorkspace(adminWorkspace, QString::fromStdString(hotel->getAllBillsText()));
            if (ok) clearEdits(adminFormFields);
            break;
        case 13:
            setWorkspace(adminWorkspace, QString::fromStdString(hotel->getDetailedBillText(adminFormFields[0]->text().toInt())));
